...
使用`==`比较，它会自动转换数据类型再比较，很多时候，会得到非常诡异的结果； 使用`===`比较，它不会自动转换数据类型，如果数据类型不一致，返回`false`，如果一致，再比较。 由于JavaScript这个设计缺陷，尽量避免使用`==`比较。
...
如果一个变量没有通过`var`申明就被使用，那么该变量就自动被申明为全局变量, 可以启用用`strict模式`强制通过`var`申明变量，未使用`var`申明变量就使用的，将导致运行错误(部分浏览器支持),
